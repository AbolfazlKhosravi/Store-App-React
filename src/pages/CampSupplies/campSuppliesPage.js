...
import CampSuppliesFilter from "../../components/common/campSuppliesFilter";
import { useState } from "react";
import { useTheme } from "../../provider/themeMode";
const CampSuppliesPage = () => {
  const navigate = useNavigate();
  const theme = useTheme();
  const [filteredProducts, setFilteredProducts] = useState(
    products.campSupplies
  );
  const { cart } = useCart();
      ...
      <main id={styles[theme]}>
        <section className={styles.shoesPage}>
          <div className={styles.filterShowses}>
            ...
            <CampSuppliesFilter setFilteredProducts={setFilteredProducts} />
          </div>
...
export default CampSuppliesPage;
